...
<div class="md:flex md:items-center md:justify-between mb-6">
    ...
    <div class="mt-4 flex md:mt-0 md:ml-4 space-x-2">
        ...
        @if(auth()->user()->can('update', $observation))
        <a href="{{ route('observations.edit', $observation) }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700">
            <i class="fas fa-edit mr-2"></i> Edit
        </a>
        @endif
        @can('review', $observation)
        @if($observation->status === 'pending')
        <a href="{{ route('observations.review', $observation) }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700">
