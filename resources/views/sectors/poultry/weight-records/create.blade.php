...
<div class="md:flex md:items-center md:justify-between mb-6">
    ...
    <div class="mt-4 flex md:mt-0 md:ml-4">
        <a href="{{ $batch ? route('poultry.batches.show', $batch) : route('poultry.forms.hub') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form method="POST" action="{{ route('poultry.weight-records.store') }}" id="weightForm" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="poultry_batch_id" id="poultry_batch_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent" required onchange="updateBatchInfo()">
                        ...
                        <option value="{{ $item->id }}" data-remaining="{{ $item->current_count }}" data-age="{{ $item->age_days }}" {{ old('poultry_batch_id', $batch?->id) == $item->id ? 'selected' : '' }}>
            ...
            <div class="flex gap-3 pt-6 border-t border-gray-200">
                ...
                <a href="{{ $batch ? route('poultry.batches.show', $batch) : route('poultry.forms.hub') }}" class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-900 font-medium py-2.5 px-4 rounded-lg transition-colors text-center">
