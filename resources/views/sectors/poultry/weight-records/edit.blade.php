...
<div class="md:flex md:items-center md:justify-between mb-6">
    <div>
        ...
        <p class="text-sm text-gray-600">Update weight record for {{ $weightRecord->batch->batch_id ?? 'this batch' }}</p>
    </div>
    <div class="mt-4 flex md:mt-0 md:ml-4">
        <a href="{{ route('poultry.batches.show', $weightRecord->batch) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form method="POST" action="{{ route('poultry.weight-records.update', $weightRecord) }}" id="weightForm" class="space-y-6">
            @csrf
            @method('PUT')

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="poultry_batch_id" class="block text-sm font-medium text-gray-700">Batch</label>
                    <select name="poultry_batch_id" id="poultry_batch_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent" required onchange="updateBatchInfo()">
                        @foreach($weightRecord->batch ? [$weightRecord->batch] : [] as $item)
                        <option value="{{ $item->id }}" data-remaining="{{ $item->current_count }}" data-age="{{ $item->age_days }}" selected>
                            {{ $item->batch_id }} - {{ $item->name }} ({{ $item->current_count }} birds)
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="date" id="date" value="{{ old('date', $weightRecord->date->format('Y-m-d')) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent" required>
            ...
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <div class="grid grid-cols-2 gap-4">
                    <div><p class="text-xs text-gray-600">Batch Age</p><p class="text-lg font-semibold text-gray-900" id="batchAge">{{ $weightRecord->batch->age_days ?? 0 }} days</p></div>
                    <div><p class="text-xs text-gray-600">Remaining Birds</p><p class="text-lg font-semibold text-gray-900" id="batchRemaining">{{ $weightRecord->batch->current_count ?? 0 }} birds</p></div>
                </div>
            </div>
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                    @php $weights = $weightRecord->individual_weights ?? []; @endphp
                    @for($i = 1; $i <= 10; $i++)
                    <div>
                        <label for="weight_{{ $i }}" class="block text-xs font-medium text-gray-600 mb-1">Bird {{ $i }}</label>
                        <input type="number" name="weight_{{ $i }}" id="weight_{{ $i }}" step="0.001" min="0.001" max="5" value="{{ $weights[$i-1] ?? '' }}" placeholder="0.000" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 bird-weight-input">
                    </div>
                    @endfor
                </div>
            ...
            <div class="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-4 border border-green-200">
                ...
                <div id="cvStatusBox" class="mt-3 p-2 rounded text-sm font-medium">
                    @php
                        $cv = $weightRecord->coefficient_variation;
                        if ($cv >= 15) {
                            $statusClass = 'bg-red-100 text-red-800';
                            $statusText = '⚠️ High variation – will be saved and flagged for monitoring';
                        } elseif ($cv >= 12) {
                            $statusClass = 'bg-yellow-100 text-yellow-800';
                            $statusText = '⚠️ Caution – Monitor closely';
                        } elseif ($cv >= 10) {
                            $statusClass = 'bg-blue-100 text-blue-800';
                            $statusText = '✓ Good variation';
                        } else {
                            $statusClass = 'bg-green-100 text-green-800';
                            $statusText = '✅ Excellent uniformity';
                        }
                    @endphp
                    <span id="cvStatusText" class="inline-block px-2 py-1 rounded {{ $statusClass }}">{{ $statusText }}</span>
                </div>
            </div>

            <div>
                <label for="notes" class="block text-sm font-medium text-gray-700">Notes</label>
                <textarea name="notes" id="notes" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent">{{ old('notes', $weightRecord->notes) }}</textarea>
            </div>
            ...
            <div class="flex gap-3 pt-6 border-t border-gray-200">
                <button type="submit" class="flex-1 bg-primary-600 hover:bg-primary-700 text-white font-medium py-2.5 px-4 rounded-lg transition-colors">
                    <i class="fas fa-save mr-2"></i> Update Weight Record
                </button>
                <a href="{{ route('poultry.batches.show', $weightRecord->batch) }}" class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-900 font-medium py-2.5 px-4 rounded-lg transition-colors text-center">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateBatchInfo() {
        ...
        if (batchSelect.value && option) {
            document.getElementById('batchAge').textContent = (option.dataset.age || 0) + ' days';
            document.getElementById('batchRemaining').textContent = (option.dataset.remaining || 0) + ' birds';
        }
    }
    ...
    function calculateStats() {
        ...
        if (cv >= 15) {
            ...
            statusText.className = 'inline-block px-2 py-1 rounded bg-red-100 text-red-800';
            statusText.textContent = '⚠️ High variation – will be saved and flagged for monitoring';
        } else if (cv >= 12) {
            ...
            statusText.className = 'inline-block px-2 py-1 rounded bg-yellow-100 text-yellow-800';
            ...
            statusText.className = 'inline-block px-2 py-1 rounded bg-blue-100 text-blue-800';
            ...
            statusText.className = 'inline-block px-2 py-1 rounded bg-green-100 text-green-800';
