...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div class="bg-gradient-to-br from-amber-50 to-amber-100 rounded-lg shadow p-4 border-l-4 border-amber-500">
            <h3 class="text-sm font-medium text-amber-700"><i class="fas fa-kiwi-bird mr-1"></i>{{ auth()->user()->role === 'admin' ? 'Sell/Bird' : 'Rec. Sell/Bird' }}</h3>
        ...
        <div class="bg-gradient-to-br from-rose-50 to-rose-100 rounded-lg shadow p-4 border-l-4 border-rose-500">
            <h3 class="text-sm font-medium text-rose-700"><i class="fas fa-tag mr-1"></i>{{ auth()->user()->role === 'admin' ? 'Sell/Kg' : 'Rec. Sell/Kg' }}</h3>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-3">
                <div class="flex justify-between">
                    <span class="text-sm text-gray-600">Remaining Birds</span>
                    <span class="text-lg font-bold">{{ number_format($batch->current_count) }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-sm text-gray-600">Starting Flock</span>
                    <span class="text-lg font-bold">{{ number_format($batch->starting_flock) }}</span>
                </div>
                <div class="pt-3 border-t border-gray-100">
                    <div class="flex flex-wrap justify-between gap-2 text-sm">
                        <span class="text-red-600"><i class="fas fa-skull mr-1"></i> Mortality: {{ number_format($batch->total_mortality, 1) }}</span>
                        <span class="text-yellow-600"><i class="fas fa-cut mr-1"></i> Culls: {{ $batch->total_culls }}</span>
                        <span class="text-green-600"><i class="fas fa-drumstick-bite mr-1"></i> Slaughter: {{ $batch->total_slaughter }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-3">
                <div class="flex justify-between">
                    ...
                    <span class="text-lg font-bold">{{ format_weight($batch->current_average_weight) }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-sm text-gray-600">iFCR</span>
                    <span class="text-lg font-bold {{ $batch->current_ifcr < 1.8 ? 'text-green-600' : ($batch->current_ifcr < 2.0 ? 'text-yellow-600' : 'text-red-600') }}">{{ format_fcr($batch->current_ifcr) }}</span>
                </div>
                <div class="flex justify-between">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold text-gray-900">
                <i class="fas fa-chart-pie mr-2 text-red-500"></i> Mortality Overview
            </h3>
            <span class="text-xs text-gray-500">Helps detect unexpected flock losses</span>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="text-center p-4 bg-red-50 rounded-lg border border-red-100">
                <p class="text-xs uppercase tracking-wide text-red-700 font-medium">Total Mortality</p>
                <p class="text-3xl font-bold text-red-700 mt-1">{{ number_format($batch->total_mortality, 1) }}</p>
                <p class="text-xs text-red-600 mt-1">Birds lost to date</p>
            </div>
            <div class="text-center p-4 bg-orange-50 rounded-lg border border-orange-100">
                <p class="text-xs uppercase tracking-wide text-orange-700 font-medium">Pen Mortality</p>
                <p class="text-3xl font-bold text-orange-700 mt-1">{{ number_format($batch->pen_mortality, 1) }}</p>
                <p class="text-xs text-orange-600 mt-1">Deaths recorded in this batch</p>
            </div>
            <div class="text-center p-4 bg-blue-50 rounded-lg border border-blue-100">
                <p class="text-xs uppercase tracking-wide text-blue-700 font-medium">Historical Carried</p>
                <p class="text-3xl font-bold text-blue-700 mt-1">{{ number_format($batch->historical_mortality - $batch->pen_mortality, 1) }}</p>
                <p class="text-xs text-blue-600 mt-1">Mortality inherited via transfers</p>
            </div>
        </div>

        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="p-4 bg-gray-50 rounded-lg">
                <p class="text-xs uppercase text-gray-600 font-medium">Mortality Rate</p>
                <p class="text-2xl font-bold mt-1 {{ $batch->mortality_rate > 7 ? 'text-red-600' : ($batch->mortality_rate > 4 ? 'text-yellow-600' : 'text-green-600') }}">
                    {{ number_format($batch->mortality_rate, 2) }}%
                </p>
                <p class="text-xs text-gray-500 mt-1">
                    {{ number_format($batch->total_mortality, 0) }} ÷ {{ number_format($batch->starting_flock, 0) }} birds
                </p>
            </div>
            <div class="p-4 bg-gray-50 rounded-lg">
                <p class="text-xs uppercase text-gray-600 font-medium">Remaining Birds</p>
                <p class="text-2xl font-bold mt-1 text-gray-900">{{ number_format($batch->current_count) }}</p>
                <p class="text-xs text-gray-500 mt-1">
                    Starting {{ number_format($batch->starting_flock) }} − losses − transfers
                </p>
            </div>
        </div>
        ...
        <div class="mt-3 text-xs text-gray-500 border-t border-gray-200 pt-3">
            <p class="mb-1">
                ...
                <strong>Pen Mortality</strong> — deaths recorded directly in this batch
            </p>
            <p class="mb-1">
                <span class="inline-block w-3 h-3 bg-blue-400 rounded mr-1"></span>
                <strong>Historical Carried</strong> — mortality inherited when birds were transferred in
            </p>
            <p>
                <span class="inline-block w-3 h-3 bg-red-400 rounded mr-1"></span>
                <strong>Total Mortality</strong> — Pen + Historical (used for mortality rate %)
            </p>
        </div>
    </div>
    ...
    <div class="bg-red-50 border border-red-200 rounded-xl p-4">
        ...
        <div class="space-y-2">
            ...
            <div class="flex items-start p-3 bg-white rounded border border-red-100">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead>
                            <tr>
                                ...
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">Sample</th>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">Avg. Weight</th>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">CV</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="px-4 py-2 text-sm">{{ $record->date->format('M d, Y') }}</td>
                                <td class="px-4 py-2 text-sm">{{ $record->birds_weighed }}</td>
                                <td class="px-4 py-2 text-sm font-medium">{{ format_weight($record->average_weight) }}</td>
                                <td class="px-4 py-2 text-sm">{{ number_format($record->coefficient_variation, 2) }}%</td>
                                <td class="px-4 py-2">{!! cv_status_badge($record->cv_status) !!}</td>
                            </tr>
                            @empty
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead>
                            <tr>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">Date</th>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">Feed Used</th>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">Cost/kg</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="px-4 py-2 text-sm">{{ $record->date->format('M d, Y') }}</td>
                                <td class="px-4 py-2 text-sm">{{ number_format($record->feed_used, 2) }} kg</td>
                                <td class="px-4 py-2 text-sm">{{ format_currency($record->feed_cost_per_kg) }}</td>
                                <td class="px-4 py-2 text-sm font-medium">{{ format_currency($record->total_feed_cost) }}</td>
                            </tr>
                            @empty
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="px-4 py-2 text-sm">{{ $expense->date->format('M d, Y') }}</td>
                                <td class="px-4 py-2 text-sm">{{ ucfirst($expense->category) }}</td>
                                <td class="px-4 py-2 text-sm">{{ $expense->description }}</td>
                                <td class="px-4 py-2 text-sm font-medium">{{ format_currency($expense->amount) }}</td>
                            </tr>
                            @empty
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead>
                            <tr>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">Date</th>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">Mortality</th>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">Culls</th>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">Slaughter</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="px-4 py-2 text-sm">{{ $record->date->format('M d, Y') }}</td>
                                <td class="px-4 py-2 text-sm text-red-600">{{ $record->mortality }}</td>
                                <td class="px-4 py-2 text-sm text-yellow-600">{{ $record->culls }}</td>
                                <td class="px-4 py-2 text-sm text-green-600">{{ $record->slaughter }}</td>
                                <td class="px-4 py-2 text-sm text-gray-500">{{ Str::limit($record->notes ?? '—', 40) }}</td>
                            </tr>
                            @empty
                            <tr><td colspan="5" class="px-4 py-8 text-center text-gray-500">No flock records</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 text-center">
            ...
            <a href="{{ route('poultry.forms.weight-record.create', ['batch' => $batch->batch_id]) }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-primary-600 hover:bg-primary-700">
                <i class="fas fa-plus mr-2"></i> Add Weight
            </a>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 text-center">
            ...
            <a href="{{ route('poultry.forms.feed-record.create', ['batch' => $batch->batch_id]) }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-primary-600 hover:bg-primary-700">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 text-center">
            <div class="w-16 h-16 rounded-full bg-purple-100 flex items-center justify-center mx-auto mb-4">
                <i class="fas fa-kiwi-bird text-purple-600 text-xl"></i>
            </div>
            <h4 class="text-lg font-semibold text-gray-900 mb-2">Update Flock</h4>
            <a href="{{ route('poultry.forms.flock-record.create', ['batch' => $batch->batch_id]) }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-primary-600 hover:bg-primary-700">
                <i class="fas fa-plus mr-2"></i> Update Flock
            </a>
        </div>
    </div>
</div>
